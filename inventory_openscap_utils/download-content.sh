...
function DownloadGitHubReleases() {
    ...
    do
        echo "[>]Writing to $DOWNLOAD_PATH/$AUTHOR_REPO..."
        mkdir -p "$DOWNLOAD_PATH"/"$AUTHOR_REPO" > /dev/null
        ARTIFACTS=$(curl -s https://api.github.com/repos/"$AUTHOR_REPO"/releases/latest | awk -F '"' '/browser_download_url/{print $4}')
        for URL in $ARTIFACTS
        do
            ARCHIVE=$(basename "$URL")
            if [[ ! "$ARCHIVE" =~ $IGNORE_LIST ]] && [[ ! -e "$DOWNLOAD_PATH"/"$AUTHOR_REPO"/"$ARCHIVE" ]]; then
                echo "[*]Downloading $ARCHIVE..."
                curl --silent -L "$URL" --output "$DOWNLOAD_PATH"/"$AUTHOR_REPO"/"$ARCHIVE"
            elif [[ ! "$ARCHIVE" =~ $IGNORE_LIST ]] && [[ -e "$DOWNLOAD_PATH"/"$AUTHOR_REPO"/"$ARCHIVE" ]]; then
